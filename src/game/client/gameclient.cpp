...
void CGameClient::OnConsoleInit()
{
	...
	//Some DDNet CLient Commands:
	Console()->Register("pause", "", CFGFLAG_CLIENT, ConDDPause, this, "Pause/unpause local game");
...
void CGameClient::OnConnected()
{
	...
	for (int i = 0; i < NUM_NETOBJTYPES; i++)
	{
		unsigned int Offset = Client()->GetNetObjOffset(CurrentServerInfo.m_DDNetVersion, i);

		if (Offset > 0)
			Client()->SnapSetOffset(i, Offset);
	}
	dbg_msg("version", "Serverversion: %d", CurrentServerInfo.m_DDNetVersion);
...
void CGameClient::SendKill(int ClientID) //TODO: Remove obsolete ClientID (Also why spam the server with kill command when ur client knows we are dead?)
{
	CNetMsg_Cl_Kill Msg;
	Client()->SendPackMsg(&Msg, MSGFLAG_VITAL);
}

void CGameClient::SendDDPause()
{
	CNetMsg_Cl_DD_Pause Msg;
	Client()->SendPackMsg(&Msg, MSGFLAG_VITAL);
}
...
void CGameClient::ConDDPause(IConsole::IResult *pResult, void *pUserData)
{
	((CGameClient*)pUserData)->SendDDPause();
}
